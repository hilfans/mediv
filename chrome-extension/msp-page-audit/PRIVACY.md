...
pihak ketiga** (PageSpeed Insights, dan kalau diaktifkan, Gemini milik
Google serta Bing Webmaster Tools milik Microsoft). **Data yang diproses
ekstensi ini tidak pernah melewati atau disimpan di server PT Mitra
Solusindo Pratama** — semuanya berjalan lokal di perangkat pengguna atau
langsung ke penyedia API tersebut memakai API key milik pengguna sendiri.
...
| Cek Backlink (Bing Webmaster Tools, opsional) | URL situs yang ingin dicek backlink-nya | Dikirim langsung dari browser Anda ke **Bing Webmaster Tools API** milik Microsoft, memakai API key milik Anda sendiri (bukan milik PT MSP). Fitur ini sepenuhnya opsional dan hanya mengembalikan data untuk situs yang sudah diverifikasi kepemilikannya di akun Bing Webmaster Tools pemilik API key — kalau Anda tidak mengisi API key ini, tidak ada data yang pernah dikirim ke Bing. |
...
Semua hasil audit, pengaturan, dan API key (PageSpeed Insights, Gemini,
maupun Bing Webmaster Tools) disimpan **hanya di penyimpanan lokal
browser Anda** (`chrome.storage.local`) — tersimpan di perangkat Anda
sendiri, tidak disinkronkan ke akun Google/Microsoft Anda, dan tidak
pernah dikirim ke server PT Mitra Solusindo Pratama atau pihak ketiga
mana pun selain permintaan API langsung yang dijelaskan di Bagian 1.
...
- **Bing Webmaster Tools API** (hanya kalau Anda mengaktifkan fitur "Cek
  Backlink") — menerima URL situs yang ingin dicek backlink-nya. Dipanggil
  memakai API key yang Anda buat dan masukkan sendiri lewat halaman
  Options ekstensi. Tunduk pada
  [Kebijakan Privasi Microsoft](https://privacy.microsoft.com/id-id/privacystatement)
  dan persyaratan layanan Bing Webmaster Tools.
...
- API key (PageSpeed Insights, Gemini & Bing Webmaster Tools) bisa
  dihapus kapan saja lewat halaman Options ekstensi (tombol "Hapus").
...
- Fitur berbasis AI (Gemini) dan Cek Backlink (Bing Webmaster Tools)
  sepenuhnya opt-in — tidak berjalan otomatis, hanya saat Anda mengklik
  tombol terkait secara eksplisit dan hanya kalau API key-nya sudah diisi.
